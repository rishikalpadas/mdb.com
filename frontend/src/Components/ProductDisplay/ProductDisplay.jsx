...
const ProductDisplay = (props) => {
 ...
 const waterMarkedImage = async (image,_id) => {
    try {
      ...
      const watermarkText = `©mdb_${_id.slice(20)}.com`;
      ctx.font = "32px Arial";
  ...
  useEffect(() => {
    const generateWatermarkedImages = async () => {
      if (product?.image) {
        const watermarked = await waterMarkedImage(product.image,product._id);
        setMainWatermarkedImage(watermarked);
        // Create array for thumbnail images
        const thumbnailPromises = Array(4).fill().map(() => waterMarkedImage(product.image,product._id));
